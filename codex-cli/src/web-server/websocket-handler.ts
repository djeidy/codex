...
import { shouldAutoAnalyze, generateAutoAnalysisMessage, type SessionFile } from '../utils/agent/auto-analysis.js';
...
export class WebSocketHandler {
  ...
  private buildSessionContext(sessionId: string): string {
    ...
    let context = 'You are a helpful AI assistant specializing in log analysis and troubleshooting. You have access to session files and troubleshooting guides (TSGs) to help diagnose issues.\n\n';
  ...
  initialize(): void {
    ...
    // Handle TSG messages
    this.socket.on('tsg:create', (data: TSGCreateMessage) => this.handleTSGMessage(data));
    this.socket.on('tsg:list', (data: TSGMessage) => this.handleTSGMessage(data));
    this.socket.on('tsg:select', (data: TSGSelectMessage) => this.handleTSGMessage(data));
    this.socket.on('tsg:delete', (data: TSGMessage) => this.handleTSGMessage(data));
    this.socket.on('tsg:upload', (data: TSGUploadMessage) => this.handleTSGMessage(data));
    this.socket.on('tsg:get-files', (data: TSGMessage) => this.handleTSGMessage(data));
    
    // Handle session file management, uploads, and TSG messages through message event
    this.socket.on('message', (data: SessionFileDeleteMessage | SessionFileListMessage | SessionFilePreviewMessage | SessionFileUploadMessage | TSGMessage, callback?: (response?: { status: string; error?: string }) => void) => {
      log(`📨 Socket received message event, type: ${data?.type}, has callback: ${!!callback}`);
      this.handleMessage(data).then(() => {
        ...
        if (callback) {
          callback({ status: 'error', error: error instanceof Error ? error.message : 'Unknown error' });
        }
      });
    });
  ...
  private async handleTSGCreate(message: TSGCreateMessage): Promise<void> {
    log(`🎯 handleTSGCreate called with message: ${JSON.stringify(message, null, 2)}`);
    ...
    log(`📁 Creating TSG with name: ${name}, description: ${description || 'none'}`);
  ...
  private async handleSessionFileUpload(message: SessionFileUploadMessage): Promise<void> {
    const { sessionId, data } = message;
    const { files } = data;
    
    log(`📤 Handling session file upload for session ${sessionId}, ${files.length} files`);
  ...
  private async handleMessage(message: SessionFileDeleteMessage | SessionFileListMessage | SessionFilePreviewMessage | SessionFileUploadMessage | TSGMessage): Promise<void> {
    try {
      log(`📩 Received message type: ${message.type}`);
      
      switch (message.type) {
        case 'session:upload':
          await this.handleSessionFileUpload(message as SessionFileUploadMessage);
          break;
  ...
  private async handleSessionFileDelete(message: SessionFileDeleteMessage): Promise<void> {
    const { sessionId, data } = message;
    const { fileName } = data;
  ...
  private async handleSessionFileList(message: SessionFileListMessage): Promise<void> {
    const { sessionId } = message;
  ...
  private async handleSessionFilePreview(message: SessionFilePreviewMessage): Promise<void> {
    const { sessionId, data } = message;
